...
const Fotterslider: React.FC = () => {
  ...
  const handleProductClick = () => {
    navigate("/shop"); // Navigate to the /shop page when a product is clicked
  };
    ...
    <section className="container mx-auto px-4 py-12 relative bg-[#FFF9F3]">
      <div className="relative">
        ...
        <div
          ...
          className="flex overflow-x-auto scrollbar-hide space-x-0"
          onScroll={handleScroll}
        >
          {products.map((product) => (
            <motion.div
              key={product.id}
              className="flex-shrink-0 text-center w-64 cursor-pointer" // Make it clear the product is clickable
              animate={{ x: "-100%" }}
              transition={{ repeat: Infinity, ease: "linear", duration: 12 }}
              onClick={handleProductClick} // Add onClick handler here
            >
